...
{
  const [leftId, setLeftId] = useState<string>("34");
  const [rightId, setRightId] = useState<string>("34");
  ...
  const handleFusion = async () => 
  {
    ...
    try {
      ...
      setImageUrl(url);   // ← Cuando llega la imagen, se activará el useEffect
    } catch (error) {
              ...
              <div className="h-64 flex items-center justify-center text-gray-400 italic mb-4">
                ...
                {imageUrl && (
                  <img
                    ...
                    onLoad={() => {
                      // Desbloquea después de X segundos de mostrar la imagen
                      setTimeout(() => {
                        setIsWorking(false);
                      }, 3000); // Cambia este valor a los segundos que quieras
                    }}
                  />
                )}
              </div>
